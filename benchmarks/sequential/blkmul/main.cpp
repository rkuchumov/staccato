...
void seq_blkmul(Block *A, Block *B, Block *R, size_t n, bool do_mul = true);
void seq_add(Block *A, Block *B, Block *R, size_t n);

void seq_mul(Block *A, Block *B, Block *R, size_t n)
{
	if (n <= 1) {
		R->mul(A, B);
		return;
	...
	seq_blkmul(A+0*q, B+0*q, l+0*q, q);
	seq_blkmul(A+0*q, B+1*q, l+1*q, q);
	seq_blkmul(A+2*q, B+0*q, l+2*q, q);
	seq_blkmul(A+2*q, B+1*q, l+3*q, q);

	auto r = new Block[n];

	seq_blkmul(A+1*q, B+2*q, r+0*q, q);
	seq_blkmul(A+1*q, B+3*q, r+1*q, q);
	seq_blkmul(A+3*q, B+2*q, r+2*q, q);
	seq_blkmul(A+3*q, B+3*q, r+3*q, q);
...
void seq_add(Block *A, Block *B, Block *R, size_t n)
{
	...
	seq_blkmul(A+0*q, B+0*q, R+0*q, q, false);
	seq_blkmul(A+1*q, B+1*q, R+1*q, q, false);
	seq_blkmul(A+2*q, B+2*q, R+2*q, q, false);
	seq_blkmul(A+3*q, B+3*q, R+3*q, q, false);
}

void seq_blkmul(Block *A, Block *B, Block *R, size_t n, bool do_mul) {
	if (do_mul)
		seq_mul(A, B, R, n);
	else
		seq_add(A, B, R, n);
