...
void seq_matmul(
	...
	if (m >= n && n >= p) {
		size_t m1 = m >> 1;
		seq_matmul(A, B, C, m1, n, p, ld, add);
		seq_matmul(A + m1 * ld, B, C + m1 * ld, m - m1, n, p, ld, add);
	} else if (n >= m && n >= p) {
		size_t n1 = n >> 1;
		seq_matmul(A, B, C, m, n1, p, ld, add);
		seq_matmul(A + n1, B + n1 * ld, C, m, n - n1, p, ld, true);
	} else {
		size_t p1 = p >> 1;
		seq_matmul(A, B, C, m, n, p1, ld, add);
		seq_matmul(A, B + p1, C + p1, m, n, p - p1, ld, add);
	}
}
