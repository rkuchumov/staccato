...
bool dispatcher<T>::balance(size_t node)
{
	...
	if (!found_max || load_max == 0) {
		if (node)
			m_events[MIGRATION_NO_MAX_1]++;
		else
			m_events[MIGRATION_NO_MAX_0]++;
		return false;
		for (size_t i = 0; i < m_nworkers; ++i) {
			worker_state *w = m_workers + i;
	...
	if (!found_min) {
		if (node)
			m_events[MIGRATION_NO_MIN_1]++;
		else
			m_events[MIGRATION_NO_MIN_0]++;
		return false;
	}
