...
bool steals(size_t log_size, size_t exec_time)
{
	...
	thread t1(stealing_worker, pool, &start, &stop, &sum_1, &nstolen_1);
	thread t2(stealing_worker, pool, &start, &stop, &sum_2, &nstolen_2);
...
bool take_put_steals(size_t log_size, size_t exec_time, bool resize)
{
	size_t size = 1 << log_size;
	task_deque *pool = new task_deque( (resize) ? 1 : log_size );
...
int main()
{
	TEST("Concurrent steals (2^0 tasks)", steals(0, 1));
	...
	TEST("Take, put and concurrent steals, no resize (2^20 tasks)", take_put_steals(16, 1, true));
	TEST("Take, put and concurrent steals (2^20 tasks)", take_put_steals(16, 1, false));
